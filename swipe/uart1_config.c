#include "uart_config.h"//@@@注意：这个包含文件如果放到"uart.h"之下，报错
#include "uart.h"
#include "pdma.h"
...
//		printf("=====================\r\n");
	...
	if( UART_GET_INT_FLAG(UART1, UART_INTSTS_THREIF_Msk) )//发送缓存空中断
	{
		if(tx !=0 )
		{
			...
			if(tx->size < tx->len)
			{
				UART1->DAT = tx->buf[tx->size++];
			}
			else
			{
				UART_DISABLE_INT(UART1, UART_INTEN_THREIEN_Msk);//关发送中断
				return;
			}
		}
		else
...
uint8_t _uart1_send(UART_DATA *pt_tx , UART_DATA** pt_rx, UART_RECV_CALLBACK callback)   
{
	...
	*pt_rx = (UART_DATA*)&rx;
	...
	#if(0) //使用发送中断
//	UART_Write(UART1, (uint8_t*)tx->buf,tx->len);
	UART_WRITE(UART1, tx->buf[tx->size++]);
