...
@函数指针
*/
typedef uint8_t(*IREADER_QUEUE_SEND)(IREADER_MSG msg);//队列发送
typedef BaseType_t(*IREADER_QUEUE_GET)(void *const msg, TickType_t time);//队列取数据
typedef BaseType_t(*UART_SEM_SEND)(void);//信号量发送
typedef BaseType_t(*UART_SEM_GET)(uint32_t time);

extern IREADER_QUEUE_SEND ireader_queue_send[];//队列信息发送数组
extern IREADER_QUEUE_GET ireader_queue_get[];//队列信息获取数组
